...
#
ORGANIZATION_DB=`echo "SELECT database_name FROM users WHERE username='${CARTO_USERNAME}'" | psql -U postgres -t carto_db_development`
...
for user in `echo "SELECT 'development_cartodb_user_' || id FROM users WHERE organization_id = (SELECT id FROM organizations WHERE name='$ORGANIZATION')" | psql -U postgres -t carto_db_development`
do
